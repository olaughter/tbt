...
from tbt.definitions.definition_objects import Definition, Pipelines, Pipeline
...
class DefinitionsParser:
    ...
    def parse(self):
        ...
        logging.info(f"Found: {len(self.definitions)} tbt files, totalling {len(self.pipelines)} pipelines")
            ...
            with open(definition.path, "r") as f:
                ...
                for k, v in pipeline_definitions.items():
                    if k in all_pipelines:
                        logging.error(f"Found pipeline: {k}, in definition files:\n - {f.name} \n - {all_pipelines[k].definition.path}")
    ...
    def sort_pipelines(self, pipelines: Pipelines) -> Pipelines:
        ...
        if not nx.is_directed_acyclic_graph(graph):
            ...
            raise ValueError(f"Found circular dependencies in files: {broken_files}, cycles: {cycles}")
        ...
        for pipeline in pipelines.values():
            if pipeline.depends_on:
                if not isinstance(pipeline.depends_on, list):
                    raise ValueError(f"depends_on must be a list for: {pipeline.label}, in {pipeline.definition.path}")
                for dependency in pipeline.depends_on:
                    if dependency not in pipelines:
                        raise ValueError(f"Dependency: {dependency}, for pipeline: {pipeline.label}, in {pipeline.definition.path}, does not exist")
                    edges.append((dependency, pipeline.label))
        return edges

    def _get_files_with_cycles(self, graph: DiGraph, pipelines: Pipelines) -> tuple[list[str], list[tuple[str]]]:

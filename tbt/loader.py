...
from tbt.meta_types import *  # noqa
from tbt.meta_types.tbt_step import tbtStep
from tbt.step_types.destinations import *  # noqa
from tbt.step_types.sources import *  # noqa
from tbt.step_types.transforms import *  # noqa
...
class Loader:
    def __init__(self, meta_type: str) -> None:
        ...
        if meta_type not in meta_type_options:
            raise ValueError(
                f"Unknown meta type {meta_type}, expected one of: {meta_type_options}"
            )
        self.meta_type_cls = meta_type_options[meta_type]

    def load_available(
        self, tbt_step_cls: types.ModuleType
    ) -> dict[str, type(tbtStep)]:
    ...
    def step(self, step):
        ...
        if step not in available_steps:
            raise ValueError(
                f"Unknown step type {step}, expected one of: {available_steps}"
            )
